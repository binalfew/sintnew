...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create sintnew-9e8d
  fly apps create sintnew-9e8d-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app sintnew-9e8d
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app sintnew-9e8d-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'sintnew-9e8d-staging' does not match app name in config file 'sintnew-9e8d'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name sintnew-9e8d-db
  fly postgres attach --postgres-app sintnew-9e8d-db --app sintnew-9e8d

  fly postgres create --name sintnew-9e8d-staging-db
  fly postgres attach --postgres-app sintnew-9e8d-staging-db --app sintnew-9e8d-staging
  ```
